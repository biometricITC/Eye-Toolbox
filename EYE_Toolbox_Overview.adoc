...
The following table specifies the maximum amount of stored artefacts that can be used for each type. Note that contacts lenses cannot be stored outside of a wet environment and so cannot be left with the photos as a complete artefact.
